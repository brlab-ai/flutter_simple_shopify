...
class ShopifyStore with ShopifyError {
  ...
  Future<List<Product>> getProductsByIds(List<String> idList,
    ...
    var newResponse = {
      'edges': List.generate((response != null ? response['nodes']??[] : []).length,
          (index) => {'node': response != null ? response['nodes'][index] : []})
    };
  ...
  Future<List<Metafield>> getMetafieldsFromProduct(
    ...
    return (result.data?['productByHandle']['metafields']['edges'] as List)
